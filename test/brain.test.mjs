...
import { applyFeedback, mergeCandidate, rebuildBrain, searchBrain } from "../src/brain.mjs";
import { readYaml, writeYamlAtomic } from "../src/contracts.mjs";

test("Brain deduplicates evidence, isolates projects, and rebuilds from canonical files", async () => {
  ...
  try {
    ...
    await writeYamlAtomic(resolve(hub, ".local", "run-index", `${runId}.yaml`), { run_id: runId, run_root: runRoot, project_id: binding.projectId, project_root: first });
    ...
    const sharedRecord = await readYaml(resolve(hub, "brain", "patterns", `${shared.id}.yaml`));
    ...
    await rebuildBrain({ hubRoot: hub });
    const localResult = await searchBrain({ hubRoot: hub, input: query(binding.projectId, "만료 경계", ["만료"]) });
    assert.ok(localResult.cards.some((card) => card.id === created.id));
    const secondBinding = await registry.resolve(second);
    const isolated = await searchBrain({ hubRoot: hub, input: query(secondBinding.projectId, "expiry boundary", ["expiry"]) });
    ...
    const unrelated = await searchBrain({ hubRoot: hub, input: query(secondBinding.projectId, "Plan a React dashboard accessibility and rendering performance improvement", ["React", "accessibility", "rendering", "performance"]) });
    assert.deepEqual(unrelated.cards, []);

    const lockPath = resolve(hub, "brain", ".store.lock");
    ...
    const interrupted = resolve(hub, "brain", "patterns", "interrupted.yaml");
    await writeFile(interrupted, "schema_version: 1\nid: interrupted\n");
    await assert.rejects(rebuildBrain({ hubRoot: hub }), /Invalid pattern record/);
    ...
    await unlink(resolve(first, ".codex-system", "patterns", `${created.id}.yaml`));
    await rebuildBrain({ hubRoot: hub, projectId: binding.projectId });
    const deleted = await searchBrain({ hubRoot: hub, input: query(binding.projectId, "만료 경계", ["만료"]) });
    assert.ok(deleted.cards.every((card) => card.id !== created.id));
  } finally {
