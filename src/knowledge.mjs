...
export async function finalizeRun({ dataRoot, hubRoot, runId }) {
  dataRoot ??= hubRoot;
  const index = await readRunIndex(dataRoot, runId);
...
export async function mergeCandidate({ dataRoot, hubRoot, binding, candidate, evidence, expectedRevision = null }) {
  dataRoot ??= hubRoot;
  validateCandidate(candidate);
  const observation = await validateEvidence(evidence, binding);
  const scope = candidate.scope === "shared" ? "shared" : "project";
  const store = resolve(dataRoot, "knowledge");
  await assertMaintenanceInactive(dataRoot);
  ...
  try {
    ...
    const observations = current?.observations ?? (current?.evidence ?? []).map(legacyObservation);
    const duplicate = observations.some((entry) => entry.id === observation.id);
    const candidateRelations = await resolveRelations({ dataRoot, binding, candidate, evidence, currentId: id });
    ...
    const pattern = {
      ...
      status: current?.status === "needs_revalidation" ? current.status : evidence.outcome === "fail" ? "provisional" : evidenceStatus(duplicate ? observations : [...observations, observation]),
      applies_when: candidate.applies_when.trim(),
      recommended: candidate.recommended.trim(),
      avoid: candidate.avoid.trim(),
      observations: duplicate ? observations : [...observations, observation],
...
export async function searchKnowledge({ dataRoot, hubRoot, input }) {
  dataRoot ??= hubRoot;
  validateSearchInput(input);
  const stores = [resolve(dataRoot, "knowledge")];
...
export async function rebuildKnowledge({ dataRoot, hubRoot }) {
  dataRoot ??= hubRoot;
  return [await rebuildStore(resolve(dataRoot, "knowledge"))];
}

export async function migrateLegacyKnowledge({ dataRoot, legacyRoot, fixtureRoots = [] }) {
  const source = resolve(legacyRoot, "brain", "patterns");
  const destination = resolve(dataRoot, "knowledge", "patterns");
  await mkdir(destination, { recursive: true });
  let names = [];
  try { names = (await readdir(source)).filter((name) => name.endsWith(".yaml")); }
  catch (error) { if (error.code === "ENOENT") return { migrated: [], skipped_fixtures: [] }; throw error; }
  const migrated = [];
  const skippedFixtures = [];
  for (const name of names.sort()) {
    const legacy = await readYaml(resolve(source, name));
    const evidence = legacy.evidence ?? [];
    const fixtureOnly = evidence.length > 0 && evidence.every((entry) => entry.source_run_root && fixtureRoots.some((root) => isWithin(resolve(root), resolve(entry.source_run_root))));
    if (fixtureOnly) { skippedFixtures.push(legacy.id); continue; }
    const record = {
      schema_version: 2,
      id: legacy.id,
      revision: legacy.revision,
      scope: legacy.scope,
      ...(legacy.project_id ? { project_id: legacy.project_id } : {}),
      kind: legacy.kind,
      stages: legacy.stages ?? [],
      tags: legacy.tags ?? [],
      status: "needs_revalidation",
      applies_when: legacy.applies_when,
      recommended: legacy.recommended,
      avoid: legacy.avoid,
      observations: evidence.map(legacyObservation),
      relations: legacy.relations ?? [],
      assessments: [...(legacy.assessments ?? []), { verdict: "needs_revalidation", reason: "Migrated legacy evidence lacks a complete retained execution excerpt." }],
      updated_at: new Date().toISOString(),
    };
    validatePattern(record);
    const target = resolve(destination, name);
    try {
      const current = await readYaml(target);
      if (JSON.stringify(current) !== JSON.stringify(record)) throw new Error(`Knowledge migration conflict: ${target}`);
    } catch (error) { if (error.code === "ENOENT") await writeYamlAtomic(target, record); else if (!/migration conflict/.test(error.message)) throw error; else throw error; }
    migrated.push(record.id);
  }
  await rebuildKnowledge({ dataRoot });
  return { migrated, skipped_fixtures: skippedFixtures };
}

export async function applyFeedback({ dataRoot, hubRoot, runId, feedback }) {
  dataRoot ??= hubRoot;
  ...
  const index = await readRunIndex(dataRoot, runId);
  await assertMaintenanceInactive(dataRoot);
  const runLock = await acquireOwnedLock(resolve(dataRoot, "state", "locks", `feedback-${hash(runId).slice(0, 16)}.lock`), { run_id: runId, operation: "feedback" }, `Feedback is already active for ${runId}`);
  try {
  await assertMaintenanceInactive(dataRoot);
  ...
  for (const store of stores) {
    ...
    try {
      ...
      for (const name of (await readdir(root)).filter((item) => item.endsWith(".yaml"))) {
        ...
        const linked = (pattern.observations ?? []).filter((entry) => entry.origin?.run_id === runId);
        if (linked.length === 0) continue;
        if (criterionIds.length && !linked.some((entry) => entry.criterion_ids?.some((id) => criterionIds.includes(id)))) continue;
        const nextStatus = feedback.verdict === "rejected" ? "needs_revalidation" : await evidenceAvailable(pattern) ? evidenceStatus(pattern.observations) : "needs_revalidation";
...
async function validateEvidence(evidence, binding) {
  ...
  const selected = evidence.check_ids.map((id) => byId.get(id));
  return {
    id: `observation-${hash(canonicalEvidence(evidence)).slice(0, 16)}`,
    basis: "execution",
    outcome: evidence.outcome,
    criterion: (evidence.criterion_ids ?? []).join(", ") || "assessed execution criterion",
    criterion_ids: evidence.criterion_ids ?? [],
    check: evidence.check_ids.join(", "),
    check_ids: evidence.check_ids,
    result: selected.map((check) => `${check.id}: ${check.status}${compactExcerpt(check.stderr || check.stdout)}`).join("; "),
    environment: `${process.platform}/${process.arch}`,
    artifact_digest: evidence.artifact_revision,
    observed_at: new Date().toISOString(),
    origin: { project_id: evidence.project_id, run_id: evidence.run_id },
    provenance: { source_run_root: evidence.source_run_root },
  };
}

function validatePattern(pattern) {
  if (pattern?.schema_version !== 2 || !pattern.id || !["project", "shared"].includes(pattern.scope) || !Number.isInteger(pattern.revision) || pattern.revision < 1) throw new Error(`Invalid knowledge record: ${pattern?.id ?? "unknown"}`);
  if (pattern.scope === "project" && !pattern.project_id) throw new Error(`Project pattern lacks project_id: ${pattern.id}`);
  if (!Array.isArray(pattern.observations) || !Array.isArray(pattern.relations)) throw new Error(`Knowledge record lacks observations or relations: ${pattern.id}`);
  for (const observation of pattern.observations) if (!observation.id || observation.basis !== "execution" || !["pass", "fail"].includes(observation.outcome) || !observation.check || !observation.result || !observation.artifact_digest || !observation.origin?.run_id) throw new Error(`Invalid observation in ${pattern.id}`);
...
function validateSearchInput(input) {
  if (!input || input.schema_version !== 1 || typeof input.project_id !== "string" || typeof input.task_summary !== "string" || typeof input.stage !== "string") throw new Error("Knowledge search requires schema_version, project_id, task_summary, and stage");
}
...
function evidenceStatus(observations) {
  if (observations.some((entry) => entry.outcome === "fail")) return "provisional";
  return new Set(observations.filter((entry) => entry.outcome === "pass" && entry.basis === "execution" && entry.criterion_ids?.length).map((entry) => `${entry.origin?.project_id}/${entry.origin?.run_id}`)).size >= 2 ? "validated" : "provisional";
}

async function acquireStoreLock(store) {
  const path = resolve(store, ".store.lock");
  return acquireOwnedLock(path, { operation: "knowledge-write" }, `Knowledge store is busy: ${store}`);
}
...
async function evidenceAvailable(pattern) {
  return pattern.observations.length > 0 && pattern.observations.every((observation) => observation.basis === "execution" && ["pass", "fail"].includes(observation.outcome) && observation.check && observation.result && observation.artifact_digest && observation.origin?.run_id);
}
...
async function resolveRelations({ dataRoot, binding, candidate, evidence, currentId }) {
  const resolved = [];
  for (const relation of candidate.relations ?? []) {
    ...
    const targets = [];
    for (const store of [resolve(dataRoot, "knowledge")]) {
      try {
...
function legacyObservation(evidence) {
  return {
    id: `observation-${hash(canonicalEvidence(evidence)).slice(0, 16)}`,
    basis: evidence.basis,
    outcome: evidence.outcome,
    criterion: (evidence.criterion_ids ?? []).join(", ") || "legacy assessed criterion",
    criterion_ids: evidence.criterion_ids ?? [],
    check: (evidence.check_ids ?? []).join(", ") || "legacy check",
    check_ids: evidence.check_ids ?? [],
    result: "Legacy evidence requires revalidation.",
    environment: "legacy",
    artifact_digest: evidence.artifact_revision ?? "unknown",
    observed_at: evidence.observed_at ?? new Date(0).toISOString(),
    origin: { project_id: evidence.project_id, run_id: evidence.run_id },
    provenance: { source_run_root: evidence.source_run_root ?? null },
  };
}
...
async function readRunIndex(dataRoot, runId) {
  try { return await readYaml(resolve(dataRoot, "state", "run-index", `${runId}.yaml`)); }
  catch (error) { if (error.code !== "ENOENT") throw error; }
  return readYaml(resolve(dataRoot, ".local", "run-index", `${runId}.yaml`));
}
